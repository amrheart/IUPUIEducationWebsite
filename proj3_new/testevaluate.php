...
<body>
	...
	<table>
	    ...
	    <?php
		$query = "SELECT * FROM REPORT  WHERE SiteSupApprovalStatus = 0 AND Rejected = 0";
		$stmt = $con->prepare($query);
		$stmt->execute();
		$row_num = 0;
		while($row = $stmt->fetch(PDO::FETCH_OBJ)){
			echo "<tr>" . "<td><a href=" . '"' . "testviewreport.php?" . "student=" . strval($row_num) . '"' . ">" . "View Report </a></td>". "<td>" . $row->StudentFirstName . "</td>" . "<td>" . $row->HoursCompleted . "</td><tr>";
			$row_num += 1;
		}
	    ?>
	</table>
